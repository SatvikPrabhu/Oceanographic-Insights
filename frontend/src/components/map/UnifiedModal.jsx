...
export default function UnifiedModal({ selected, data, viewMode, onClose }) {
  ...
  return (
    <div className="absolute inset-0 z-[2000] flex items-end justify-end bg-black/35 p-4 md:items-center md:justify-center">
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-ink-900 shadow-panel">
        <div className="flex items-start justify-between border-b border-white/10 px-5 py-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-cyan-200/70">Unified spatial insight</p>
        ...
        <div className="space-y-4 px-5 pb-5">
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-200/80">Physical water stats</h3>
            {ocean.length ? (
              <ul className="mt-2 max-h-28 space-y-1 overflow-auto text-sm text-ink-50">
                {ocean.slice(0, 6).map((row) => (
                  <li key={row._id} className="rounded-lg bg-ink-800/70 px-3 py-2">
              ...
              <p className="mt-2 text-sm text-ink-400">No ocean stations in this neighbourhood.</p>
            )}
          </section>

          <section>
            <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-200/80">Local catch history</h3>
            {fisheries.length ? (
              ...
              <p className="mt-2 text-sm text-ink-400">No landings recorded near this coordinate.</p>
            )}
          </section>

          <section>
            <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-fuchsia-200/80">Detected DNA species</h3>
            {species.length ? (
              <div className="mt-2 flex flex-wrap gap-2">
                {species.map((name) => (
                  <span
                    key={name}
                    className="rounded-full border border-fuchsia-400/30 bg-fuchsia-400/10 px-2.5 py-1 text-xs text-fuchsia-100"
                  >
                    {name}
                  </span>
                ))}
              ...
              <p className="mt-2 text-sm text-ink-400">No eDNA matches at this location.</p>
            )}
